...
namespace TPL_TM.Pages
{
    ...
    public class Move_ItemModel : PageModel
    {
        ...
        public Move_ItemModel(NetSuiteClient netSuiteClient, IConfiguration configuration)
        {
        ...
        public IActionResult OnPost()
        {
            try
            {
                ...
                TaskInfo task = new TaskInfo
                {
                    ...
                    Assign_To = "From " + Request.Form["Source_Location"] + " To " + Request.Form["Destination_Location"],
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(DefaultConnection))
                {
                    ...
                    string sqlString = "select COUNT(*) as task_count from task_manager Where Task_Type = 'Move Item'";
                    using (NpgsqlCommand sqlCmd = new NpgsqlCommand(sqlString, connection))
                    {
                        using (NpgsqlDataReader reader = sqlCmd.ExecuteReader())
                        {
                            ...
                            if (reader.Read() && !reader.IsDBNull(0))
                            {
                                ...
                                task.Ref_No = "MI"+(currentQty + 1).ToString("00000#");
                            }
                            else 
                            {
                                task.Ref_No = "MI"+(1).ToString("00000#");
                            }
