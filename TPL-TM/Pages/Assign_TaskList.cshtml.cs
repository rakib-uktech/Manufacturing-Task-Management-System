...
{
    [Authorize]
    public class Assign_TaskListModel : PageModel
    {
        ...
        public Assign_TaskListModel(IConfiguration configuration)
        {
        ...
        public void OnGet()
        {
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(DefaultConnection))
                {
                    ...
                    string sql = "SELECT * from task_manager Where task_type='Assembly Build' and status='A'";
