...
{
    [Authorize]
    public class Request_InventoryListModel : PageModel
    {
        ...
        public Request_InventoryListModel(IConfiguration configuration)
        {
        ...
        public void OnGet()
        {
            try
            {
                ...
                using (NpgsqlConnection connection = new NpgsqlConnection(DefaultConnection))
                {
                    ...
                    string sql = "SELECT * from task_manager Where task_type='Request Inventory' and status='A'";
